...
// Middleware
app.use(cors({
  origin: process.env.FRONTEND_URL || 'www.urbancraftglobal.com',
  credentials: true
}));
app.use(express.json());

// Initialize Resend (will use RESEND_API_KEY from env if not passed)
let resend;
try {
  resend = new Resend(process.env.RESEND_API_KEY);
} catch (error) {
  console.warn('⚠️  Resend not initialized. Set RESEND_API_KEY in .env file.');
...
app.post('/api/contact', async (req, res) => {
  try {
    ...
    if (!resend) {
      throw new Error('Resend is not initialized. Please set RESEND_API_KEY in your .env file.');
    }

    const fromEmail = process.env.FROM_EMAIL || 'support@urbancraftglobal.com';
...
Reply directly to: ${email}
    ...
    // Send both emails
    console.log(`[${new Date().toISOString()}] Attempting to send emails via Resend...`);
    console.log(`[${new Date().toISOString()}] User email: ${email}`);
    console.log(`[${new Date().toISOString()}] Company email: ${companyEmail}`);
    ...
    console.log(`[${new Date().toISOString()}] ✅ User confirmation email sent successfully:`, userResult.data?.id);
    console.log(`[${new Date().toISOString()}] ✅ Company notification email sent successfully:`, companyResult.data?.id);
    ...
    if (error.message?.includes('API key')) {
      errorMessage = 'Resend API key is missing or invalid. Please check your RESEND_API_KEY in .env file.';
...
app.listen(PORT, () => {
  ...
  if (!process.env.RESEND_API_KEY) {
    console.warn('⚠️  WARNING: RESEND_API_KEY is not set. Email functionality will not work.');
  } else {
